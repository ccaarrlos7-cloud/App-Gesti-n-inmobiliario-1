...
import { Mail, Lock, Loader2, ArrowRight, ShieldCheck, Home, Users, TrendingUp, Building2, ArrowLeft } from 'lucide-react';
      ...
      <div className="relative z-10 flex w-full h-full flex-col xl:flex-row overflow-hidden">
        ...
        <div className="w-full xl:w-[550px] h-full flex items-center justify-center p-3 sm:p-4 lg:p-8 shrink-0 min-h-0">
          <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-3xl shadow-2xl p-5 sm:p-6 lg:p-8 border border-white/20 dark:border-slate-800/50 flex flex-col justify-center shrink-0 min-h-0">
            ...
            {isForgotPassword ? (
              <form onSubmit={handleForgotPassword} className="space-y-3 shrink-0">
                ...
                >
                  {loading
                    ...
                    : <><span>{isEs ? 'Enviar enlace' : 'Send link'}</span><ArrowRight className="h-4 w-4 sm:h-5 sm:w-5" /></>}
                </button>
                <button
                  type="button"
                  onClick={goBackToLogin}
                  className="w-full flex items-center justify-center gap-2 py-2 text-sm font-semibold text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 transition-colors"
                >
              ...
              <form onSubmit={handleSubmit} className="space-y-3 shrink-0 flex flex-col justify-center min-h-0">
                ...
                {isLogin && (
                  <div className="flex justify-end shrink-0">
                    <button
                      ...
                      className="text-xs sm:text-sm text-[#FACC15] hover:text-[#EAB308] font-semibold transition-colors"
                    >
                ...
                >
                  {loading
                    ? <Loader2 className="animate-spin h-4 w-4 sm:h-5 sm:w-5" />
                    : <><span>{isLogin ? (isEs ? 'Iniciar sesión' : 'Sign In') : (isEs ? 'Registrarse' : 'Sign Up')}</span><ArrowRight className="h-4 w-4 sm:h-5 sm:w-5" /></>}
                </button>
              </form>
            )}
            ...
            <div className="mt-4 pt-3 sm:pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-center gap-2 text-slate-500 dark:text-slate-400 shrink-0">
              ...
              <div className="text-[10px] sm:text-xs">
                ...
                <p>{isEs ? 'Utilizamos medidas de seguridad' : 'We use security measures'}</p>
              </div>
            </div>
          </div>
        </div>
